package MyBatisThorough.mybatis2;

import javax.sql.DataSource;

/**
 * 解读Mybatis数据源实现
 * 那么我们先来看看 不使用池化的数据源实现 它叫做UnpooledDataSource 我们先来看看源码:
 ...
 *                  }
 * 首先它将数据库的连接信息也给添加到Properties对象中进行存放 并交给下一个doGetConnection来处理 套完就完事了呗 接着来看下一层源码:
 ...
 * 因此它的玩法非常之高级 但是 再高级的玩法 我们都要拿下
 *
 * 首先注意 它存放了一个UnpooledDataSource 此对象是在构造时就被创建 其实创建Connection还是依靠数据库驱动创建 我们可以猜测
 ...
 *                                  }
 *                               // 如果已经没有多余的连接可以分配 那么就检查一下活跃连接数量是否达到最大的分配上限 如果没有 就new一个
 ...
 *                                          }
 *
 *                                          // 这里就根据之前的连接对象直接new一个新的连接(注意: 使用的还是之前的Connection对象 只是被重新封装了)
 ...
 *                                          if (!conn.getRealConnection().getAutoCommit()) { // 清理之前遗漏的事务操作
 ...
 *                          }
 *                      }
 * 经过上面一顿操作之后 我们可以得到以下信息:
 *      如果最后得到了连接对象(有可能是从空闲列表中得到 有可能是直接创建的新的 还有可能是经过回收策略回收得到的) 那么连接(Connection)对象一定会被放在活跃列表中(state.activeConnection)
 *
 * 那么肯定有一个疑问 现在我们已经知道获取一个链接会直接进入到活跃列表中 那么 如果一个链接被关闭 又会发生什么事情呢 我们来看看 此方法返回之后 会调用getProxyConnection来获取一个代理对象 实际上就是PooledConnection类:
 ...
 *                              // 这里并不会真的关闭连接(这也是为什么用代理) 而是调用之前数据源的pushConnection方法 将此连接改为空闲状态
 ...
 *                          }
 *                      }
 * 这样 我们就完全了解了Mybatis池化数据源的执行流程了 只不过 无论Connection管理方式如何变换 无论数据源再高级
 * 我们要知道 它最终都会使用DriverManger来创建连接对象 而最终使用的也是DriverManager提供的Connection对象
 */
public class Main {
