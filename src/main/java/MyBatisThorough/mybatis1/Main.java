package MyBatisThorough.mybatis1;
 ...
 *
 * 而学习了Mybatis之后 我们就不用再去使用DriverManager为我们连接对象 而是直接使用Mybatis为我们提供的SqlSessionFactory工具类来获取对应的SqlSession 通过会话对象去操作数据库
 *
 * 那么 他到底是如何封装JDBC的呢 我们可以试着猜想一下 会不会是Mybatis每次都是帮助我们调用DriverManager来实现的数据库连接创建? 我们可以看看Mybatis的源码:
 ...
 *                          // 生成事务 (根据我们的配置 会默认生成JdbcTransaction) 这里是关键 我们看到这里用到了environment.gteDataSource()方法
 ...
 *                      return var8;
 *
 * 也就是说 我们的数据源配置信息 存放在了Transaction对象中 那么现在我们只需要知道执行器是如何执行SQL语句的 我们就知道到底如何创建Connection对象了
 * 就需要获取数据库链接信息了 那么我们来看看 这个DataSource到底是个什么:
 ...
 *
 *                      }
 * 我们发现 它是在javax.sql定义的一个接口 它包括了两个方法 都是用于获取连接的 因此 现在我们可以断定 并不是通过之前DriverManager的方法获取连接了
 * 而是使用DataSource的实现类来获取的 因此 也就正式引入到我们这一节的话题了:
 *      数据库链接的建立和关闭是极其耗费系统资源的操作 通过DriverManager获取的数据库连接 一个数据库连接对象均对应一个物理数据库连接
 *      每次操作都打开一个物理连接 使用完后立即关闭连接 频繁的打开 关闭连接会持续消耗网路资源 造成整个系统性能低下
 *
 ...
 *
 * 当然 也可以使用传统的即用即连的方式获取Connection对象 Mybatis为我们提供了几个默认的数据源实现 我们之前之前一直在使用的是官方的默认配置 也就是池化数据源:
